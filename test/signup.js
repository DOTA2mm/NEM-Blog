...
var testName2 = 'awesome'
describe('signup', function () {
  describe('POST /signup', function () {
    var agent = request.agent(app) // persist cookie when redirct
    beforeEach(function (done) { // hook: 在本区块的每个测试用例之前执行
      // 创建一个用户
      User.create({
        name: testName1,
        password: '123456',
    ...
    afterEach(function (done) { // hook: 在本区块的每个测试用例之后执行
      // 删除测试用户
    ...
    // 两次密码不一致
    it('not same password', function (done) {
      agent
        .post('/signup')
        .type('form')
        .attach('avatar', path.join(__dirname, 'avatar.jpg'))
        .field({name: testName2, gender: 'f', bio: 'noder', password: '123456', repassword: ''})
        .redirects()
        .end(function (err, res) {
          if (err) return done(err)
